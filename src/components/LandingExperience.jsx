...
import { Bike, CarFront, Check, Droplet, Package, Search, Sparkles, X } from "lucide-react";
...
const categories = [
  ...
  },
  {
    key: "traslados",
    name: "Traslados",
    keywords: ["traslado", "traslados", "transporte", "viaje", "viajes"],
    icon: CarFront,
  },
  {
    key: "limpieza",
    ...
    <>
      <section id="categorias" className="mt-1.5 scroll-mt-4" aria-labelledby="categories-title">
        ...
        <div className="relative mt-2">
          ...
          <input
            ...
            className="h-12 w-full rounded-xl border border-slate-200 bg-white pl-11 pr-11 text-sm text-slate-900 shadow-[0_4px_14px_rgba(15,23,42,0.08)] outline-none transition focus:border-blue-400 focus:ring-2 focus:ring-blue-100"
            placeholder="Buscar servicios..."
          />
          {query ? <button type="button" onClick={() => setQuery("")} aria-label="Limpiar búsqueda" className="absolute right-2 top-1/2 grid size-9 -translate-y-1/2 place-items-center rounded-lg text-slate-500 hover:bg-slate-100 focus-visible:outline-2 focus-visible:outline-blue-600"><X aria-hidden="true" size={18} /></button> : null}
        </div>
        ...
        {visibleCategories.length ? (
          <div className="mt-3 flex snap-x snap-mandatory gap-2 overflow-x-auto overscroll-x-contain px-3 pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" aria-label="Categorías de servicio">
            {visibleCategories.map(({ key, name, icon: Icon, active }) => active ? (
              <a key={key} href="#servicios" aria-label={`${name}, disponible`} className="w-[7.5rem] shrink-0 snap-start rounded-xl border border-blue-200 bg-blue-50 p-2.5 text-blue-700 shadow-sm focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-600">
                <div className="flex items-start justify-between gap-1"><Icon aria-hidden="true" size={21} /><span className="rounded-full bg-white px-1.5 py-0.5 text-[0.5rem] font-black uppercase tracking-wide text-blue-700">Disponible</span></div>
                <strong className="mt-1.5 block truncate text-xs text-[#101a5c]">{name}</strong>
              </a>
            ) : (
          ...
          <div className="mt-3 rounded-xl border border-dashed border-slate-300 bg-white px-4 py-6 text-center">
            ...
            <button type="button" onClick={() => setQuery("")} className="mt-3 min-h-11 rounded-xl bg-blue-50 px-4 text-sm font-extrabold text-blue-700 focus-visible:outline-2 focus-visible:outline-blue-600">Limpiar búsqueda</button>
          </div>
        )}
      </section>
      ...
      {showsMandados ? (
        <section id="servicios" className="mt-5 scroll-mt-4" aria-labelledby="services-title">
          ...
          <p className="mt-1 text-[0.72rem] font-semibold text-slate-500">{availabilityLabel}</p>
          <div className="mt-3 grid grid-cols-2 rounded-xl bg-slate-100 p-1" role="group" aria-label="Ordenar servicios">
            {[
              ...
              return <button key={value} type="button" onClick={() => setSortMode(value)} aria-pressed={selected} className={`flex min-h-11 items-center justify-center gap-1.5 rounded-lg px-2 text-xs font-extrabold transition focus-visible:outline-2 focus-visible:outline-blue-600 ${selected ? "bg-white text-blue-700 shadow-sm ring-1 ring-slate-200" : "text-slate-600"}`}>{selected ? <Check aria-hidden="true" size={15} strokeWidth={3} /> : null}{label}</button>;
            })}
          </div>
          <div className="mt-3">

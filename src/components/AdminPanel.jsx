...
function AdminServiceThumbnail({ service }) {
  ...
  return <div className="relative grid size-14 shrink-0 place-items-center overflow-hidden rounded-xl border border-slate-200 bg-slate-100">{service.logoUrl && !failed ? <Image src={service.logoUrl} alt={`Logo de ${service.name}`} fill sizes="56px" className="bg-white object-contain p-1.5" onError={() => setFailed(true)} /> : <ImageIcon className="text-slate-400" size={24} aria-hidden="true" />}</div>;
}

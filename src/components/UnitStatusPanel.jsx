        ...
        {unit.availabilityMode === "manual" ? <p className="mt-4 rounded-xl bg-blue-50 p-3 text-sm leading-5 text-slate-700">Tu disponibilidad permanece como la indiques hasta que termine el periodo actual o vuelvas a cambiarla.</p> : (
          <>
            <div className="mt-5 grid gap-4">
              {schedule.map((day) => <div key={day.day} className="rounded-xl border border-slate-200 p-3"><div className="flex items-center justify-between"><p className="font-extrabold text-slate-900">{DAYS[day.day - 1]}</p><button type="button" disabled={day.blocks.length >= 2 || pending !== null} onClick={() => updateDay(day.day, (current) => ({ ...current, blocks: [...current.blocks, { start: "08:00", end: "18:00" }] }))} className="flex min-h-9 items-center gap-1 text-xs font-bold text-[var(--brand-blue)] disabled:opacity-40"><Plus size={15} /> Horario</button></div>{day.blocks.length ? <div className="mt-2 grid gap-2">{day.blocks.map((block, index) => <div key={index} className="grid grid-cols-[1fr_auto_1fr_auto] items-center gap-2"><input aria-label={`Inicio ${DAYS[day.day - 1]}`} type="time" value={block.start} onChange={(event) => updateDay(day.day, (current) => ({ ...current, blocks: current.blocks.map((item, itemIndex) => itemIndex === index ? { ...item, start: event.target.value } : item) }))} className="min-h-11 min-w-0 rounded-lg border border-slate-300 px-2" /><span>—</span><input aria-label={`Fin ${DAYS[day.day - 1]}`} type="time" value={block.end} onChange={(event) => updateDay(day.day, (current) => ({ ...current, blocks: current.blocks.map((item, itemIndex) => itemIndex === index ? { ...item, end: event.target.value } : item) }))} className="min-h-11 min-w-0 rounded-lg border border-slate-300 px-2" /><button type="button" aria-label={`Eliminar horario de ${DAYS[day.day - 1]}`} onClick={() => updateDay(day.day, (current) => ({ ...current, blocks: current.blocks.filter((_, itemIndex) => itemIndex !== index) }))} className="grid size-10 place-items-center rounded-lg text-slate-500"><X size={17} /></button></div>)}</div> : <p className="mt-2 text-sm text-slate-400">Cerrado</p>}</div>)}
            </div>
